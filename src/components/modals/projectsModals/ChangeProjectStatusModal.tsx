import {
  ...
  import { changeProjectStatus, deleteProject } from "../../../services/projects.services";
  ...
  const ChangeProjectStatusModal = ({ isOpen, onClose }) => {
    ...
    const submitChangeStatus = async () => {
      if (selectedProject) {
        try {
          const response = await changeProjectStatus(user.token, selectedProject.id);
          ...
          toast({
            ...
            description: "The project status couldn't be changed. try again later",
            status: "error",
            duration: 2000,
            isClosable: true,
          });
